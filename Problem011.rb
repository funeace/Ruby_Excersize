...
# 上を１段ずつ配列にする
init.chomp.each_line do |number|
 numbers.push(number.chomp.split(" ").map(& :to_i))
end

# 横の結果
  numbers.each do |number|
    number.length.times do|i|
      result.push(number[i..i+3].inject(1){ |result, item| result * item })
    end
  end

# 縦の結果 transposeで縦横を変換
  numbers.transpose.each do |number|
    number.length.times do|i|
      result.push(number[i..i+3].inject(1){ |result, item| result * item })
    end
  end
